...
fn fixture(hidden_size: usize) -> tempfile::TempDir {
    ...
    let wordlevel = tokenizers::models::wordlevel::WordLevel::builder()
        .vocab([("[UNK]".into(), 0), ("hello".into(), 1), ("world".into(), 2)]
            .into_iter().collect())
        .unk_token("[UNK]".into())
        .build().unwrap();
    MmTokenizer::new(wordlevel).save(dir.path().join("tokenizer.json"), false).unwrap();
    dir
}
...
fn install_other_factory() {
    ...
    assert!(GLOBAL_MODEL_FACTORY.set(ModelFactory::new(Device::Cpu)).is_ok());
}
...
fn assert_embeddings(hidden_size: usize) {
    ...
    assert!(batch.iter().all(|row| row.len() == hidden_size && row.iter().all(|v| v.is_finite())));
}
...
fn run_case(case: &str) {
    ...
    match case {
        "factory_first" | "factory_first_direct" => {
            let dir = fixture(8);
            ...
            assert!(GLOBAL_MODEL_FACTORY.get().unwrap().get_mmbert_model().is_none());
            assert!(STANDALONE_MMBERT.get().is_some());
            assert_embeddings(8);
            assert_repeated_init_is_noop();
        }
        "mmbert_first" | "mmbert_first_direct" => {
            let dir = fixture(8);
            let model_path = path(&dir);
            ...
            assert!(init_embedding_models(std::ptr::null(), std::ptr::null(), true));
            assert!(GLOBAL_MODEL_FACTORY.get().unwrap().get_mmbert_model().is_some());
            assert!(STANDALONE_MMBERT.get().is_none());
            assert_repeated_init_is_noop();
            assert!(STANDALONE_MMBERT.get().is_none(), "reinitialization must not load a duplicate");
        }
        "concurrent_mmbert" => {
            let dir = fixture(8);
            ...
            assert!(GLOBAL_MODEL_FACTORY.get().unwrap().get_mmbert_model().is_some());
            assert!(STANDALONE_MMBERT.get().is_none(), "concurrent init must not load a duplicate");
            assert_repeated_init_is_noop();
        }
        "concurrent_other_factory" => {
            ...
            thread::scope(|scope| {
                ...
                assert!(GLOBAL_MODEL_FACTORY.get().is_none(), "the losing initializer must not load/publish early");
        ...
        "failed_load_retry" | "failed_standalone_retry" => {
            let dir = fixture(8);
            let model_path = path(&dir);
            if case == "failed_standalone_retry" { install_other_factory(); }
            let tokenizer = dir.path().join("tokenizer.json");
            let bytes = std::fs::read(&tokenizer).unwrap();
            std::fs::remove_file(&tokenizer).unwrap();
            assert!(!combined(&model_path, true));
            assert!(get_mmbert_refs().is_none(), "failed init must not publish a partial model");
            std::fs::write(tokenizer, bytes).unwrap();
            assert!(combined(&model_path, true), "failed initialization must be retryable");
            assert_embeddings(8);
        }
        "poisoned_gate" => {
            assert!(thread::spawn(|| {
            ...
            assert!(!init_embedding_models_with_mmbert(std::ptr::null(), std::ptr::null(), std::ptr::null(), true));
            assert!(!init_embedding_models(std::ptr::null(), std::ptr::null(), true));
...
fn embedding_init_order_regressions() {
    ...
    for case in ["factory_first", "factory_first_direct", "mmbert_first", "mmbert_first_direct",
        "concurrent_mmbert", "concurrent_other_factory", "all_entrypoints_share_gate",
        "failed_load_retry", "failed_standalone_retry", "poisoned_gate"] {
        let mut child = Command::new(std::env::current_exe().unwrap())
            .args(["--exact", &format!("{}::embedding_init_order_regressions", module_path!().split_once("::").unwrap().1), "--nocapture"])
            .env(CHILD_CASE, case)
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                ...
                panic!("initialization case {case} timed out (possible deadlock): {}\n{}",
                    String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
            }
            thread::sleep(Duration::from_millis(10));
        }
        let output = child.wait_with_output().unwrap();
        assert!(output.status.success(), "initialization case {case} failed:\n{}\n{}",
            String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
        assert!(String::from_utf8_lossy(&output.stdout).contains("1 passed;"), "child case {case} did not execute exactly one test");
